...
typedef enum _ASINetworkErrorType {
    ...
    //add by ly 2012-03-22 当服务返回码<200, >400时 调用修改后的ASI逻辑,因此返回的错误码为ASIHttpStateError, 使用者可以调用 request.responseStateCode 检查状态
    ASIHttpStateError = 12    
...
@interface ASIHTTPRequest : NSOperation <NSCopying> {
    ...
    BOOL shouldPlusRangeInRequestHeader;
}
...
//add by cc 设置SSL证书
- (void)setSSLSecurityLevel:(CFStringRef *)level;
...
@property (assign) BOOL shouldPlusRangeInRequestHeader;

///仅限测试用途，默认为nil。若设置该值，connect时会使用该ip地址，而请求header中的host依然是请求url中的host
@property (nonatomic,assign,setter=setSSLSecurityLevel:) CFStringRef *sslSecurityLevel;//add by cc   SSL证书设置

@property (nonatomic, retain) NSString *fakeConectionIP;
